...
```primeros_6_meses``` y ```numero_par_menor12```:

```{r}
primeros_6_meses <- c("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio")
numero_par_menor12 <- c(2,4,6,8,10,12)
```
...
y _paquetes_ destinados al análisis estadístico de la información. Por ejemplo, 
...
```{r}
LISTA <- list(primeros_6_meses, numero_par_menor12, ensenada, fit)
```
...
Antes de iniciar nuestra breve discusión sobre percepción remota dejemos nuestros sistemas listos para manejar información _geo-referenciada_. Es decir, instalemos los _paquetes_ de R que serán usados en este proyecto. Un paquete de R es una serie de funciones escritas con la intención de resolver múltiples problemas asociados a un tema en específico y que han sido empaquetadas para su fácil distribución y uso.

Hay decenas de paquetes para el manejo y análisis de información geo-referenciada con R, nosotros haremos uso de los siguientes:
...
se despliega. Alternativamente, podemos utilizar la función ```install.packages()``` 
...
Las ideas que discutiremos a continuación son aplicables a cualquier acervo de 
...
que no incurriremos en un error tipográfico que nos impida acceder al archivo, 
o archivos, deseado.
...
del mosaico ó _tile_^[Ver [Sinusiodal Tile Grid](https://modis-land.gsfc.nasa.gov/MODLAND_grid.html).] 
...
exitósamente en septiembre de 2021 y se espera que para mediados de febrero de 
...
los sensores ópticos registran la radiación solar reflejada por la Tierra. De acuerdo 
a la teoría ondulatoria de la radiación electromagnética, ésta se transmite de 
