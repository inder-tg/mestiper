...
Con todo esto, ahora podemos producir bandas de confianza alrededor de los valores
ajustados por la regresión armónica:
...
<!-- Con todo esto, ahora podemos producir bandas de predicción alrededor de algunos  -->
<!-- puntos de interés. Por ejemplo, la última fecha de la serie de tiempo de NDVI de  -->
<!-- Manantlán (libre de incendios) tiene un dato faltante, entonces podemos utilizar  -->
<!-- esta sección para estimar el valor faltante y construir una banda de predicción  -->
<!-- para este valor. -->

<!-- ::: {.small} -->

<!-- ```{r predictionBand, echo=FALSE, fig.cap="Banda de predicción basado en el modelo de regresión armónica (a mediano plazo).", fig.align='center'} -->
<!-- numFreq <- 2 -->

<!-- # yCopy <- y # can be used for HANTS -->
<!-- # y[length(y)] <- NA  -->
<!-- # y <- yCopy -->

<!-- y_na <- y -->
<!-- y_na[length(y_na)] <- NA -->
<!-- # W <- as.numeric(y_na[97:108]) # useful for plotting purposes -->

<!-- yMat <- vecToMatrix(y,23) -->
<!-- y[length(y)] <- median(yMat[1:8,23]) -->
<!-- Y <- as.numeric(y[97:108]) -->

<!-- # Y es un vector con mediciones NDVI  -->
<!-- # Y[12] se rellenó con la mediana de obs. históricas  -->
<!-- # de la misma fecha -->
<!-- temp <- haRmonics(y=Y, method="harmR", numFreq=2, delta=0) -->

<!-- fit <- temp$fitted -->

<!-- sigma <- sqrt(sum( ( Y - fit )^2 )/( 12 -  (2*numFreq+1) )) -->

<!-- cb_upper <- fit + sigma * sqrt(w) * qnorm(1-(0.05)/2) -->

<!-- cb_lower <- fit - sigma * sqrt(w) * qnorm(1-(0.05)/2) -->

<!-- # y0 es el predictor de NDVI para la última fecha  -->
<!-- y0 <- sum(temp$amplitude * cos( (2 * pi * c(0:2) * L)/L - temp$phase )) -->

<!-- pb_upper <- y0 + sigma * sqrt(1+w[1]) * qt(1-(0.05)/2, df=12-(2*numFreq+1) ) -->
<!-- pb_lower <- y0 - sigma * sqrt(1+w[1]) * qt(1-(0.05)/2, df=12-(2*numFreq+1) ) -->

<!-- UPPER <- c(cb_upper[-12], pb_upper) -->
<!-- LOWER <- c(cb_lower[-12], pb_lower) -->

<!-- yRan <- range(UPPER, LOWER, W, na.rm = TRUE) -->
<!-- yRan[1] <- floor(yRan[1]) -->
<!-- yRan[2] <- ceiling(yRan[2]) -->
...
<!-- par(mar=c(4,4,2,2), new=TRUE) -->
<!-- plot(1:12, UPPER, type="l", lty=3, col="blue", ylim=yRan, ylab="",  -->
<!--      xlab="") -->
<!-- title(paste0("PIXEL:", PIXEL), adj = 1, line = 0.5) -->

<!-- points(1:12, W, pch=20, col="lightgray") -->
<!-- lines(1:12, LOWER, type="l", lty=3, col="blue") -->
...
<!-- x <- 1:12 -->
<!-- polygon( c(x[ x >= 11 ], rev( x[ x >= 11 ] )), -->
<!--          c(UPPER[ x >= 11 ], rev(LOWER[ x >= 11 ])),  -->
<!--          col = "#BCD4E6", border = NA) -->

<!-- lines(1:12, c(fit[-12],y0), col="red") -->
<!-- ``` -->
