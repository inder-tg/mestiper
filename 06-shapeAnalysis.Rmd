...
Natural Protegida _Marismas Nacionales_ durante el periodo 2000-2018, véase la Figura \@ref(fig:marismas).
...
A largo plazo podemos apreciar la dinámica periódica de este NDMI. A corto plazo, 
...
\[
  y_t = f(t) + \varepsilon_t, \quad t = 1,\ldots, L,
\]
donde
\begin{align}
  f(t) &= a_0 + 
  ...
  b_j \, \sin \left( \frac{2\pi jt}{L} \right) \right) (\#eq:harmReg-original)\\
  &=
  c_0 +
  \sum_{j=1}^p\,c_j\,\cos\left( \frac{2\pi \, jt}{L} - \varphi_j \right), (\#eq:harmReg)
\end{align}
y el término $\varepsilon_t$ simboliza el error de medición al tiempo $t$. Este error 
se supone _aleatorio_ y en esta sección consideramos que $\varepsilon_t \sim \msc{N}(0,\sigma^2)$.

La ecuación \@ref(eq:harmReg) se obtiene haciendo uso de igualdades trigonométricas, un poco de álgebra y definiedo $c_j=\sqrt{ a_j^2 + b_j^2 }$, $\varphi_j=\mbox{arctn}(b_j/a_j)$. Posteriormente discutiremos las aplicaciones de esta ecuación, en lo que sigue nos 
concentraremos en la Ec. \@ref(eq:harmReg-original). 
...
Los parámetros en \@ref(eq:harmReg-original) son las $a_j$s, $b_j$s y $p$. Si bien existen técnicas de para determinar un valor de $p$ (**selección de modelo**), en esta sección usaremos $p=2$. Es decir, la regresión armónica empleada en esta sección utiliza 3 armónicos (el armónico cero dá lugar al parámetro $a_0$). ¿Cómo estimamos las $a_j$s y $b_j$s?

Observa que podemos utilizar algebra matricial para representar las $L$ 
observaciones de NDVI de la siguiente forma:
\begin{equation}
  \begin{pmatrix}
    y_1 \\ y_2 \\ \vdots \\ y_L
  \end{pmatrix}
...
\begin{equation}
  ...
  = \argMin_{\bs{\vartheta}\in \R^5}\, \| \bs{y} - \bs{X}\,\bs{\vartheta} \|^2.
\end{equation}
...
# set.seed(10)
SAMPLE <- 1975 # sample(1:nrow(mohinora$values), 1)
# set.seed(NULL)

y <- mohinora_DATA_interpol_rTp$values[SAMPLE,]
yMat <- vecToMatrix(y,23)
z <- c(t(yMat)) * 1e-4
y <- y * 1e-4
ts_y <- ts(y, start = c(2000, 1), end = c(2023, 23), frequency = 23)

```

Abajo mostramos el ajuste vía regresión armónica de un píxel de NDVI registrado
en el Área de Protección de Flora y Fauna _Cerro Mohinora_ en Chihuahua, México. 
El periodo estudiado comprende Enero 1, 2000 a Diciembre 31, 2023. El ajuste fue producido por la función ```haRmonics``` del paquete ```geoTS```.

::: {.tiny}
```{r haRmonics, echo=c(1:5, 9:26), fig.cap="Ajuste de NDVI (2000-2023) vía regresión armónica con 2 frecuencias.", fig.align='center'}
N <- 24 # Años estudiados
...
ts_fit <- ts(fit, start = c(2000, 1), end = c(2023,23), frequency = L)
...
ts_fit <- ts(fit, start = c(2000, 1), end = c(2023, 23), frequency = L)
ts_cb_upper <- ts(cbands_upper, start = c(2000, 1), end = c(2023, 23), frequency = L)
ts_cb_lower <- ts(cbands_lower, start = c(2000, 1), end = c(2023, 23), frequency = L)
...
Hasta este punto hemos aplicado el modelo de regresión armónica suponiendo que
la varianza de la variable de interés, NDVI, es constante a lo largo del tiempo.
Esta característica suele denominarse _homoscedásticidad_ en los errores del modelo. 
Adicionalmente, hemos empleado
este modelo utilizando un número arbitrario de armónicos, $p=2$. De acuerdo a la
sección anterior, aún con estas limitantes el modelo de regresión armónica con errores
homoscedástico muestra ser útil para labores tales como la predicción.
En esta sección discutiremos el modelo de regresión armónica con errores _heteroscedásticos_
y presentaremos algunas ideas generales para seleccionar cuantitativamente el _mejor_ 
modelo.

### Heteroscedasticidad en el modelo de regresión armónica

En la sección \@ref(harmReg) presentamos el modelo de regresión armónica con errores
cuya variabilidad es constante (homoscedástico) y discutimos criterios empíricos para decidir
el número de frecuencias/armónicos utilizados en el ajuste de este modelo a datos
de NDVI. Este modelo, sin embargo, parece quedar corto para describir la dinámica
a largo plazo del NDVI.

En efecto, consideremos la curva de climatología de una serie de tiempo de NDVI en 
Cerro Mohinora:

```{r shape-climatology, fig.width=5, fig.height=4, fig.cap = "Curva de climatología de NDVI en un píxel arbitrario de Cerro Mohinora.", fig.pos="h", out.width='.6\\linewidth', echo=FALSE}

pixel <- mohinora_DATA_interpol_rTp$values[pixel_coords,]

pixel_climatology <- climatology(x=pixel, lenPeriod=23)
...
A partir de la Figura \@ref(fig:shape-climatology), es evidente que la variabilidad 
del NDVI provenientes de la primera fecha de adquisición es diferente 
a la de los valores de la fecha once, similarmente, la variabilidad de la fecha 
diez es diferente a la de la fecha diecinueve. A continuación, mostramos como adaptar 
el modelo de regresión armónica a este escenario realista en el cual la varianza 
de las observaciones varía en el tiempo.
...
\end{equation}
donde $f$ denota la función definida en \@ref(eq:harmReg-original)
y suponemos que los errores $\varepsilon_t$ son normales con media cero y varianza $\sigma_t^2$, 
es decir la varianza tomará un valor distinto en cada punto del tiempo.
Consideramos que el modelo \@ref(eq:WLS-HR) describe mejor la dinámica anual del NDVI. 

```{r var-climatology, echo=FALSE, include=FALSE, eval=FALSE}
apply(pixel_climatology$matrix, 2, sd)
```

De manera general, los parámetros $\sigma_t$, $t=1,\ldots,L$, ahora forman parte
del modelo \@ref(eq:WLS-HR) y como tal, deben ser estimados. Una manera sencilla
de estimar los valores de las $\sigma_t$'s es a través del uso de las distribuciones
de NDVI por fecha de adquisición. Es decir, a partir de la curva de climatología,
es posible obtener un estimador $\hat{\sigma}_t$ para cada $t$. Por ejemplo, 
se puede usar la desviación estándar de la distribución o bien un estimador robusto tal como la mediana del valor absoluto de las distancias entre las observaciones y la mediana de la distribución (MAD, por sus siglas en inglés), $\hat{\sigma}_t = \mbox{mad}\{ y_{t + 23 \cdot k } \mid k=0,1,\ldots,23\}$, $t=1,\ldots,L$.^[En R, la función ```mad``` permite calcular la _median absolute deviation_ un estimador robusto de la dispersión de una distribución.]

La estimación de los parámetros restantes del modelo es similar a la del caso
homoscedástico. Iniciamos notando que a partir de la Ec. \@ref(eq:harmReg-mat), sabemos que 
el modelo de regresión armónica heteroscedástica puede escribirse en forma matricial
con la diferencia que el vector de errores 
$\bs{\varepsilon}=\msc{N}(\bs{0},\bs{\Sigma})$ y $\bs{\Sigma}=\mbox{diagonal}(\sigma_1^2,\ldots,\sigma_L^2)$.^[En el modelo homoscedástico $\bs{\varepsilon}=\msc{N}(\bs{0},\sigma^2\,\bs{I}_L)$, donde $\bs{I}_L$ denota la matriz identidad de dimensiones $L \times L$.]
...
<!--     \end{pmatrix}, -->
<!-- \] -->
<!-- y $\bs{\varepsilon}=\msc{N}(\bs{0},\bs{\Sigma})$, $\bs{\Sigma}=\mbox{diagonal}(\sigma_1^2,\ldots,\sigma_L^2)$. -->

Aplicando la siguiente transformación,
$\bs{y}^\ast=\bs{\Sigma}^{-1/2}\,\bs{y}$, $\bs{X}^\ast = \bs{\Sigma}^{-1/2}\,\bs{X}$
y $\bs{\varepsilon}^\ast = \bs{\Sigma}^{-1/2}\,\bs{\varepsilon}$, ahora tenemos
la representación
\[
  \bs{y}^\ast
  =
  \bs{X}^\ast\,\bs{\beta} + \bs{\varepsilon}^\ast,
\]
donde, utilizando propiedades de la varianza de una transformación lineal, es 
fácil convencerse que la varianza de $\bs{\varepsilon}^\ast$ es igual a la matriz 
identidad de dimensiones $L \times L$. Esta característica nos permite utilizar 
la solución por mínimos cuadrados en el actual problema. A saber, en el modelo de 
regresión armónica heteroscedástica el vector de parámetros puede estimarse a través de:
\begin{align*}
  \hat{\bs{\beta}}^\ast 
  &=
  \left( (\bs{X}^\ast)^\top\, \bs{X}^\ast\right)^{-1}\left( (\bs{X}^\ast)^\top\, \bs{y}^\ast \right)\\
  &=
  \left( \bs{X}^\top\,\bs{\Sigma}^{-1}\,\bs{X} \right)^{-1}\,\bs{X}^\top\bs{\Sigma}^{-1}\,\bs{y}.
\end{align*}
De hecho, con la transformación $\bs{\Theta} \mapsto \bs{\Sigma}^{-1/2}\, \bs{\Theta}$
_mapeamos_ el modelo heteroscedástico al caso homoscedástico, y por tanto podemos emplear
la inferencia estadística desarrollada en el modelo homoscedástico en el caso 
heteroscedástico.
...
Ahora que tenemos dos modelos para describir el comportamiento de series de tiempo
de NDVI, emerge la pregunta ¿es posible determinar _cuantitativamente_ el mejor modelo?  

El **criterio de información de Akaike** (AIC, en inglés) es una cantidad que
permite evaluar un modelo ajustado a partir de balancear el número de parámetros 
y el ajuste obtenido. Por definición, el AIC de un modelo es igual a 
\[
    AIC = 2\,\kappa - 2\,\log\msf{L}(\hat{\vartheta}),
\]
donde $\kappa$ es el número de parámetros del modelo y $\msf{L}$ es la función
...
Denotemos con $\hat{\bs{\beta}}$ y $\hat{\bs{\beta}}^\ast$
los estimadores por mínimos cuadrados ordinarios (OLS) y mínimos cuadrados
ponderados (WLS) de $\bs{\beta}$ en los modelos \@ref(eq:harmReg-mat) y \@ref(eq:WLS-HR), 
respectivamente, y escribamos $\hat{y}_i=\bs{X}[i,]\hat{\bs{\beta}}$ y $\hat{y}_i^\ast=\bs{X}[i,]\,\hat{\bs{\beta}}^\ast$
para denotar los valores ajustados con estos modelos.

Suponiendo que ajustamos ambos modelos y en cada caso empleamos $p$ armónicos,
entonces $\kappa=2\,p+1$ y para el modelo homoscedástico (OLS), 
\[
    \msf{L}(\hat{\bs{\beta}}) \propto \sigma^{-L}\, 
    \mbox{exp} \{ -\frac{1}{2\sigma^2}\,\sum_{i=1}^L\,( y_i - \hat{y}_i )^2 \}.
\]
mientras que para el modelo heteroscedástico (WLS),
\[
    \msf{L}(\hat{\bs{\beta}}^\ast) \propto \prod_{i=1}^L\,\sigma_i^{-1}\, 
    \mbox{exp} \{ -\frac{1}{2}\, \sum_{i=1}^L\, \frac{ (y_i - \hat{y}_i^\ast)^2 }{\sigma_i^2} \}
\]

Es sabido que en el modelo de regresión armónica, el estimador por mínimos cuadrados
coincide con el estimador por máxima verosimilitud. Por tanto, el AIC de los modelos
homoscedástico (OLS) y heteroscedástico (WLS) son iguales a:
\begin{align*}
  AIC_{\mbox{OLS}}
  &=
  2p+1 - 2\left( -L \log(\sigma) - \frac{1}{2\sigma^2}\,\sum_{i=1}^{L} (y_i - \hat{y}_i)^2  \right)\\
  &=
  2p+1 + 2 L \log(\sigma) + \frac{L}{\sigma^2}\,\MSE[\hat{\bs{y}}]\\
  AIC_{\mbox{WLS}}
  &=
  2p+1 - 2\left( - \sum_{i=1}^{L}\log(\sigma_i) - \frac{1}{2} \sum_{i=1}^{L} 
  \frac{(y_i - \hat{y}_i^\ast)^2}{\sigma_i^2}  \right)\\
  &=
  2p+1 + 2 \sum_{i=1}^{L}\log(\sigma_i) + L\,\MSE[ \hat{\bs{y}}^\ast / \bs{\sigma} ],
\end{align*}
en la última igualdad, $\bs{\sigma} = (\sigma_1,\ldots,\sigma_L)$.

Arriba escribimos $\MSE$ para denotar la suma de los residuales al cuadrado interpretados 
como el promedio de la diferencia al cuadrado entre las observaciones $\bs{y}$ y 
su predictor $\hat{\bs{y}}$. Más aún, recordando la definición del _coeficiente 
de determinación en el modelo de regresion lineal_, el llamado $R^2$, tenemos lo siguiente
\[
  R^2
  =
  1 - \frac{\mbox{SRC}}{\mbox{STC}}
  =
  1 - \frac{n\,\MSE}{\mbox{STC}},
\]
donde $SRC$ es la suma de residuales al cuadrado, $\sum_{i=1}^{n}(y_i - \hat{y}_i)^2$,
y $STC$ es la suma total de cuadrados, $\sum_{i=1}^{n}(y_i - \bar{y})^2$, a partir
de donde se sigue que
\[
  n\,\MSE = \mbox{STC} \times (1 - R^2).
\]
Recordemos que mientras mejor es el ajuste del modelo lineal, el valor de $R^2$ es
más cercano a uno, lo cual resulta equivalente a que el $\MSE$ sea cercano a cero.
De manera inversa, si el valor de $\MSE$ es cercano a cero, entonces $R^2$ se
aproxima a uno, lo cual es evidencia de un buen ajuste del modelo.

En general, el AIC provee una métrica que busca balancear la parsimonia (el menor número
de parámetros) y la bondad de ajuste (el menor valor de $\MSE$) de un modelo. En 
términos prácticos, en presencia de dos modelos candidatos, el valor de AIC más
pequeño provee evidencia en favor del modelo con este AIC.
